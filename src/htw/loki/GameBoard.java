...
/**
 * DOKU:
 * Considered ArrayList for the linked neighbouring position because some
 * position has only 1 or 2 neighbours
 * Decided to use two dimentional array because array has better performance
 * compared to arraylist
 * becaue arraylist.get(index) is a function call and it takes more performance
 * comparing to referencing the index of an array
 * v
 * TODO
 * - Generate board neighbouring positions either in constructor or in a
 * separated function
...
public class GameBoard implements Cloneable {
	...
	public Stone getStoneFrom(int position) {
		for (Stone[] playerStone : this.stones)
			for (Stone stone : playerStone)
				if (stone.getPosition() == position)
					return stone;
		return null;
	}
